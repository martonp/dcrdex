...
func handlePendingBridges(s *RPCServer, params *RawParams) *msgjson.ResponsePayload {
	...
	if err != nil {
		return usage(pendingBridgesRoute, fmt.Errorf("error getting pending bridges: %v", err))
	}
...
func handleBridgeHistory(s *RPCServer, params *RawParams) *msgjson.ResponsePayload {
	...
	if err != nil {
		return usage(bridgeHistoryRoute, fmt.Errorf("error getting bridge history: %v", err))
	}
