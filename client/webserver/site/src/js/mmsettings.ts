...
function defaultUIConfig (baseMinWithdraw: number, quoteMinWithdraw: number) : UIConfig {
  return {
    allocation: {
      dex: {},
      cex: {}
...
export default class MarketMakerSettingsPage extends BasePage {
  ...
  constructor (main: HTMLElement, specs: BotSpecs) {
    ...
    Doc.bind(page.updateRunningButton, 'click', () => { this.updateRunningBot() })
  ...
  async configureUI () {
    ...
    Doc.setVis(quoteFeeNotTraded, page.quoteDexFeeBalanceSection)
    ...
    const oldCfg = this.originalConfig = Object.assign({}, defaultMarketMakingConfig, {
      ...
      buyPlacements: [],
      sellPlacements: [],
      uiConfig: defaultUIConfig(baseMinWithdraw, quoteMinWithdraw)
    }) as ConfigState
    ...
    if (botCfg) {
      ...
      oldCfg.uiConfig = botCfg.uiConfig || defaultUIConfig(baseMinWithdraw, quoteMinWithdraw)
    ...
    this.baseSettings.init(this.updatedConfig.baseOptions, this.specs.baseID, false)
    this.quoteSettings.init(this.updatedConfig.quoteOptions, this.specs.quoteID, true)
  ...
  quickBalanceMax (config: 'buyBuffer' | 'sellBuffer' | 'slippageBuffer' | 'buyFeeReserve' | 'sellFeeReserve') : number {
    const { buyLots, sellLots } = this.marketStuff()
    switch (config) {
      case 'buyBuffer': return 3 * buyLots
      case 'sellBuffer': return 3 * sellLots
  ...
  async updateAllocations () {
    ...
    console.log('updateAllocation', updatedConfig.uiConfig.usingQuickBalance)
    if (!updatedConfig.uiConfig.usingQuickBalance) {
      return
    }

    const {
      sellLots, buyLots, perBuyLot, perSellLot, baseID, quoteID, baseFeeAssetID, quoteFeeAssetID,
      baseIsAccountLocker, quoteIsAccountLocker, bui, qui, baseFeeUI, quoteFeeUI, numBuys, numSells
    } = this.marketStuff()

    const slippageBuffer = this.updatedConfig.uiConfig.quickBalance.slippageBuffer
    const buyLotsBuffer = this.updatedConfig.uiConfig.quickBalance.buysBuffer
    const totalBuyLots = buyLotsBuffer + buyLots
    const sellLotsBuffer = this.updatedConfig.uiConfig.quickBalance.sellsBuffer
    const totalSellLots = sellLotsBuffer + sellLots
    const buyFeeBuffer = this.updatedConfig.uiConfig.quickBalance.buyFeeReserve
    const sellFeeBuffer = this.updatedConfig.uiConfig.quickBalance.sellFeeReserve
    ...
    const canRebalance = !!specs.cexName && !!page.enableRebalance.checked
    ...
    if (this.runningBot) {
      ...
      toAlloc = toAllocateRunning(totalBuyLots, totalSellLots, slippageBuffer, buyFeeBuffer, sellFeeBuffer, perBuyLot, perSellLot,
      ...
      console.log({
        totalBuyLots, totalSellLots, slippageBuffer, buyFeeBuffer, sellFeeBuffer, perBuyLot, perSellLot,
        marketReport: this.marketReport, availableFunds, canRebalance, baseID, quoteID, baseFeeAssetID, quoteFeeAssetID,
        baseIsAccountLocker, quoteIsAccountLocker, buyFundingFees, sellFundingFees, oneTradeBuyFundingFees, oneTradeSellFundingFees
      })
      toAlloc = toAllocate(totalBuyLots, totalSellLots, slippageBuffer, buyFeeBuffer, sellFeeBuffer, perBuyLot, perSellLot,
        this.marketReport, availableFunds, canRebalance, baseID, quoteID, baseFeeAssetID, quoteFeeAssetID,
        baseIsAccountLocker, quoteIsAccountLocker, buyFundingFees, sellFundingFees, oneTradeBuyFundingFees, oneTradeSellFundingFees)
    }
    console.log('toAlloc', toAlloc)

    populateColoredBalancesTable(page.minAllocationTable, baseID, quoteID, baseFeeAssetID, quoteFeeAssetID, this.specs.cexName || '',
    ...
    for (const assetID of assets) {
      ...
      if (assetID === quoteID) {
        ...
        console.log('setting quote dex config', dexAlloc)
  ...
  quickBalanceSliderChanged (amt: number, config: 'buyBuffer' | 'sellBuffer' | 'slippageBuffer' | 'buyFeeReserve' | 'sellFeeReserve') {
    const max = this.quickBalanceMax(config)
    const input = this.quickBalanceInput(config)
    const val = Math.floor(max * amt)
    input.setValue(val)
  ...
  minTransferValidRange (asset: 'base' | 'quote') : [number, number] {
    ...
    const totalAlloc = (dex[assetID] + cex[assetID]) / ui.conventional.conversionFactor
  ...
  enableRebalanceChanged () {
    const { page } = this
    Doc.setVis(page.enableRebalance.checked, page.rebalanceSettings)
    this.updateAllocations()
  }
  ...
  marketStuff () {
    ...
    let [sellLots, buyLots, numBuys, numSells] = [1, 1, 1, 1]
    ...
    const { perBuyLot, perSellLot } = perLotRequirements(baseID, quoteID, baseFeeAssetID, quoteFeeAssetID, lotSize, quoteLot,
      this.marketReport, this.quoteMultiSplitBuffer(), baseIsAccountLocker, quoteIsAccountLocker)
    ...
    return {
      ...
      quoteLot, perBuyLot, perSellLot, sellLots, buyLots, numBuys, numSells, ...walletStuff
    }
  }
  ...
  setOriginalValues () {
    ...
    const rebalanceEnabled = cfg.uiConfig.cexRebalance
    Doc.setVis(rebalanceEnabled, page.rebalanceSettings)
    page.enableRebalance.checked = rebalanceEnabled
  ...
  async doSave () {
    ...
    if (this.runningBot) await MM.updateRunningBot(botCfg, cfg.uiConfig.allocation)
...
function populateColoredBalancesTable (
  div: PageElement, baseID: number, quoteID: number, baseFeeID: number, quoteFeeID: number, cexName: string,
...
function perLotRequirements (
  ...
  lotSize: number, quoteLot: number, marketReport: MarketReport, multiSplitBuffer: number,
  ...
  const quoteLotWithMultiSplitBuffer = quoteLot * (1 + multiSplitBuffer / 100)
  const perBuyLot: PerLot = { cex: {}, dex: {} }
  perBuyLot.dex[quoteID] = quoteLotWithMultiSplitBuffer
  console.log('quoteFees', marketReport.quoteFees.max)
...
function requiredFunds (
  ...
  quoteID: number, baseFeeID: number, quoteFeeID: number, buyFundingFees: number, sellFundingFees: number,
  oneTradeBuyFundingFees: number, oneTradeSellFundingFees: number) : AllocationResult {
  ...
  for (const assetID of assetIDs) {
    ...
    if (assetID === quoteID) {
      toAllocate.cex[assetID][0] = Math.floor(toAllocate.cex[assetID][0] * (1 + (slippageBuffer / 100)))
      toAllocate.dex[assetID][0] = Math.floor(toAllocate.dex[assetID][0] * (1 + (slippageBuffer / 100)))
    }

    if (assetID === baseFeeID) {
      toAllocate.dex[baseFeeID][0] += sellFeeBuffer * (marketReport.baseFees.max.swap + oneTradeSellFundingFees)
    ...
    if (assetID === quoteFeeID) {
      toAllocate.dex[quoteFeeID][0] += buyFeeBuffer * (marketReport.quoteFees.max.swap + oneTradeBuyFundingFees)
...
function toAllocate (
  ...
  buyFundingFees: number, sellFundingFees: number, oneTradeBuyFundingFees: number, oneTradeSellFundingFees: number
) : AllocationResult {
...
function toAllocateRunning (
  ...
  availableFunds: AvailableFunds, canRebalance: boolean, baseID: number, quoteID: number,
  baseFeeID: number, quoteFeeID: number, baseIsAccountLocker: boolean, quoteIsAccountLocker: boolean,
  runStats: RunStats, buyFundingFees: number, sellFundingFees: number, oneTradeBuyFundingFees: number, oneTradeSellFundingFees: number) : AllocationResult {
  ...
  for (const assetID of assetIDs) {
    ...
    const botDEXAvailable = runStats.dexBalances[assetID]?.available ?? 0
    const botCEXAvailable = runStats.cexBalances[assetID]?.available ?? 0
