...
func Run(t *testing.T, cfg *Config) {
	...
	// return 1 utxo.
	utxos, _, _, _, err := rig.secondWallet.FundOrder(ord)
	if err != nil {
	...
	utxos, _, _, _, _ = rig.secondWallet.FundOrder(ord)
	...
	utxos1, _, _, _, err := rig.secondWallet.FundOrder(ord)
	if err != nil {
		t.Fatalf("error funding first contract: %v", err)
	}
	// Get a separate set of UTXOs for each contract.
	setOrderValue(contractValue * 2)
	utxos2, _, _, _, err := rig.secondWallet.FundOrder(ord)
	if err != nil {
