...
func (db *tTxDB) getBridgeMint(burnTxID string) (*extendedWalletTx, error) {
	panic("getBridgeMint not implemented")
}
...
func TestCheckPendingTxs(t *testing.T) {
	...
	extendedTx := func(nonce, blockNum, blockStamp, submissionStamp uint64) *extendedWalletTx {
		pendingTx := eth.extendedTx(node.newTransaction(nonce, val), asset.Send, 1, nil, nil)
...
func TestTakeAction(t *testing.T) {
	...
	pendingTx := eth.extendedTx(node.newTransaction(0, aGwei), asset.Send, 1, nil, nil)
	...
	pendingTx = eth.extendedTx(node.newTransaction(1, aGwei), asset.Send, 1, nil, nil)
	eth.pendingTxs = []*extendedWalletTx{pendingTx}
	...
	pendingTx = eth.extendedTx(node.newTransaction(5050, aGwei), asset.Send, 1, nil, nil)
	eth.pendingTxs = []*extendedWalletTx{pendingTx}
	lostNonceAction = []byte(fmt.Sprintf(`{"txID":"%s","abandon":false,"replacementID":"%s"}`, pendingTx.ID, replacementTx.Hash()))
	if err := eth.TakeAction(actionTypeLostNonce, lostNonceAction); err == nil {
	...
	// Missing nonces
	tx5 := eth.extendedTx(node.newTransaction(5, aGwei), asset.Send, 1, nil, nil)
...
func TestSwapOrRedemptionFeesPaid(t *testing.T) {
	...
	for _, test := range tests {
		var txHash common.Hash
		if test.pendingTx != nil {
			wt := bw.extendedTx(test.pendingTx, asset.Unknown, 1, nil, nil)
...
type mockBridge struct {
	getMintInfoFunc   func(ctx context.Context, txID string) ([]byte, error)
	getMintInfoCalled chan struct{}
}

func (m *mockBridge) getMintInfo(ctx context.Context, bridgeTxID string) ([]byte, error) {
	if m.getMintInfoCalled != nil {
		m.getMintInfoCalled <- struct{}{}
	}
	return m.getMintInfoFunc(ctx, bridgeTxID)
}
...
func (m *mockBridge) bridge(txOpts *bind.TransactOpts, destAssetID uint32, amount *big.Int) (*types.Transaction, bool, error) {
	panic("not implemented")
}
func (m *mockBridge) mint(txOpts *bind.TransactOpts, mintInfoB []byte) (*types.Transaction, error) {
	panic("not implemented")
}
func (m *mockBridge) bridgeGas() uint64 { panic("not implemented") }
func (m *mockBridge) mintGas() uint64   { panic("not implemented") }
...
func TestBridgeManager(t *testing.T) {
	setupWithPendingBridges := func(t *testing.T, pendingBridges []*extendedWalletTx) (*bridgeManager, *mockBridge, chan asset.WalletNotification, *tTxDB, dex.Logger) {
		...
		mb := &mockBridge{
			...
			getMintInfoCalled: make(chan struct{}, 10),
		}

		db := &tTxDB{}
		db.pendingBridges = pendingBridges
		bm, err := newBridgeManager(context.Background(), mb, emitter, db, log)
		if err != nil {
			t.Fatalf("error creating bridge manager: %v", err)
		}

		bm.monitorInterval = 100 * time.Millisecond
	...
	t.Run("AddAndMonitor", func(t *testing.T) {
		...
		bm.addPendingBridge(burnTxID, destAssetID)

		// Wait for getMintInfo to be called at least twice
		for i := 0; i < 2; i++ {
			select {
			case <-mb.getMintInfoCalled:
				// Continue
			case <-time.After(1 * time.Second):
				t.Fatalf("timeout waiting for getMintInfo call %d", i+1)
			}
		}
	})
	...
	t.Run("SuccessfulMintInfo", func(t *testing.T) {
		...
		mintInfo := []byte("mintInfo")
		mb.getMintInfoFunc = func(ctx context.Context, txID string) ([]byte, error) {
			return mintInfo, nil
		}
		...
		case note := <-notificationChan:
			bridgeNote, ok := note.(*asset.BridgeReadyToMintNote)
			if !ok {
				t.Fatalf("expected *BridgeReadyToMintNote, got %T", note)
			}
			if bridgeNote.DestAssetID != destAssetID {
				t.Errorf("expected destAssetID %d, got %d", destAssetID, bridgeNote.DestAssetID)
			}
			if string(bridgeNote.MintInfo) != string(mintInfo) {
				t.Errorf("expected mintInfo %q, got %q", mintInfo, bridgeNote.MintInfo)
			}
		case <-time.After(1 * time.Second):
			t.Fatal("timeout waiting for BridgeReadyToMint notification")
		}
	})
	...
	t.Run("MarkAsComplete", func(t *testing.T) {
		...
		case <-mb.getMintInfoCalled:
			// Proceed
		case <-time.After(1 * time.Second):
			t.Fatal("timeout waiting for initial getMintInfo call")
		}
		...
		select {
		case <-mb.getMintInfoCalled:
			t.Error("getMintInfo was called after removing all pending bridges")
		default:
	...
	t.Run("LoadPendingBridges", func(t *testing.T) {
		...
		if bm.pendingBridges[burnTxID] != destAssetID {
