...
type bridge interface {
	...
	// bridge burns or locks the asset in order to bridge it to the destination.
	// requiresMint is true if a transaction must be executed on the destination
	// chain to mint the asset.
	bridge(txOpts *bind.TransactOpts, destAssetID uint32, amount *big.Int) (tx *types.Transaction, requiresMint bool, err error)

	// getMintInfo retrieves an attestation in order to mint the asset on the
	// destination chain. This must be called on the source chain.
	getMintInfo(ctx context.Context, bridgeTxID string) ([]byte, error)

	// mint executes a transaction on the destination chain to mint the asset.
	mint(txOpts *bind.TransactOpts, mintInfoB []byte) (tx *types.Transaction, err error)

	// bridgeGas returns the gas cost of the bridge transaction.
	bridgeGas() uint64

	// mintGas returns the gas cost of the mint transaction.
	mintGas() uint64
}
...
var usdcBridgeSupportedAssets = map[dex.Network]map[uint32]bool{
	dex.Mainnet: {
		usdcEthID:     true,
		usdcPolygonID: true,
	},
...
func isUSDCBridgeSupported(assetID uint32, net dex.Network) bool {
	supportedAssets, found := usdcBridgeSupportedAssets[net]
	if !found {
		return false
	}
...
func (b *usdcBridge) bridge(txOpts *bind.TransactOpts, destAssetID uint32, amount *big.Int) (tx *types.Transaction, requiresMint bool, err error) {
	destBridgeInfo := getUsdcBridgeInfo(destAssetID, b.net)
	if destBridgeInfo == nil {
		return nil, false, fmt.Errorf("usdc bridge info not found for assetID %d and network %s", destAssetID, b.net)
	}
...
func (b *usdcBridge) getMintInfo(ctx context.Context, brideTxID string) ([]byte, error) {
	receipt, err := b.node.transactionReceipt(ctx, common.HexToHash(brideTxID))
	if err != nil {
...
func (b *usdcBridge) mint(txOpts *bind.TransactOpts, mintInfoB []byte) (*types.Transaction, error) {
...
func (b *usdcBridge) bridgeGas() uint64 {
	// burn for deposit generally requires 102k-103k gas
	return 160_000
}

func (b *usdcBridge) mintGas() uint64 {
