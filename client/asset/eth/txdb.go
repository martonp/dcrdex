...
type txDB interface {
	...
	getBridgeMint(burnTxID string) (*extendedWalletTx, error)
}
...
type TxDB struct {
	*lexi.DB
	...
	bridgeBurnIndex *lexi.Index
	bridgeMintIndex *lexi.Index
...
// Pending bridges appear at the end of the index.
...
func NewTxDB(path string, log dex.Logger, baseChainID uint32) (*TxDB, error) {
	...
	bridgeBurnIndex, err := txs.AddIndex("bridgeburn", func(k, v lexi.KV) ([]byte, error) {
		wt, is := v.(*extendedWalletTx)
		if !is {
			return nil, fmt.Errorf("expected type *extendedWalletTx, got %T", wt)
		}
	...
	bridgeMintIndex, err := txs.AddUniqueIndex("bridgemint", func(k, v lexi.KV) ([]byte, error) {
		wt, is := v.(*extendedWalletTx)
		if !is {
			return nil, fmt.Errorf("expected type *extendedWalletTx, got %T", wt)
		}
	...
	return &TxDB{
		...
		bridgeBurnIndex: bridgeBurnIndex,
		bridgeMintIndex: bridgeMintIndex,
...
func (db *TxDB) getBridges(n int, refID *common.Hash, past bool) ([]*asset.WalletTransaction, error) {
	...
	return txs, db.bridgeBurnIndex.Iterate(nil, iterFunc, opts...)
}
...
func (db *TxDB) getPendingBridges() (txs []*extendedWalletTx, err error) {
	db.bridgeBurnIndex.Iterate(nil, func(it *lexi.Iter) error {
		wt := new(extendedWalletTx)
		err := it.V(func(vB []byte) error {
			return wt.UnmarshalBinary(vB)
		})
...
func (db *TxDB) getBridgeMint(burnTxID string) (*extendedWalletTx, error) {
	txHash := common.HexToHash(burnTxID)

	var wt extendedWalletTx
	found := false
	err := db.bridgeMintIndex.Iterate(txHash[:], func(it *lexi.Iter) error {
		found = true
