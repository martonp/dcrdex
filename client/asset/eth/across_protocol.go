...
func newAcrossBridge(ctx context.Context, cb bind.ContractBackend, node ethFetcher, chainAssetID uint32, net dex.Network, walletAddr, tokenAddr common.Address, log dex.Logger) (*acrossBridge, error) {
	chainID, found := chainAssetIDToAcrossChainID[net][chainAssetID]
	if !found {
...
func (b *acrossBridge) bridgeContractAllowance(ctx context.Context, sourceAssetID uint32) (*big.Int, error) {
	...
	tokenInfo := asset.TokenInfo(b.chainAssetID)
	if tokenInfo == nil {
		return nil, fmt.Errorf("no token info for asset %d", b.chainAssetID)
	}
