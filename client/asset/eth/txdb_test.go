...
func TestTxDB_GetBridgeMint(t *testing.T) {
	...
	t.Run("get mint transaction with valid burn ID", func(t *testing.T) {
		result, err := db.getBridgeMint(burnTxID)
		if err != nil {
			t.Fatalf("getBridgeMint failed: %v", err)
		}
		if result == nil {
	...
	t.Run("get mint transaction with unknown burn ID", func(t *testing.T) {
		unknownID := "0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef"
		result, err := db.getBridgeMint(unknownID)
		if err != nil {
			t.Fatalf("getBridgeMint should return nil, not error for unknown ID: %v", err)
		}
		if result != nil {
