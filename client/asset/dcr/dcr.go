...
func (dcr *ExchangeWallet) FundOrder(ord *asset.Order) (asset.Coins, []dex.Bytes, error) {
	...
	if err != nil {
		if !useSplit && reserves > 0 {
			...
			// This also deals with mixing wallets not having usable change. If
			// split() fails but fund() succeeds, we were scraping the barrel.
...
func (c *compositeUTXO) String() string {
	return fmt.Sprintf("%d", toAtoms(c.rpc.Amount))
}
