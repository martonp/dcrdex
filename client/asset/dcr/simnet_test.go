...
func runTest(t *testing.T, splitTx bool) {
	...
	// Grab some coins.
	utxos, _, _, _, err := rig.beta().FundOrder(ord)
	if err != nil {
	...
	utxos, _, _, _, _ = rig.beta().FundOrder(ord)
	...
	utxos1, _, _, _, err := rig.beta().FundOrder(ord)
	if err != nil {
		t.Fatalf("error funding first contract: %v", err)
	}
	// Get a separate set of UTXOs for each contract.
	setOrderValue(contractValue * 2)
	utxos2, _, _, _, err := rig.beta().FundOrder(ord)
	if err != nil {
