...
func printUsage() {
	fmt.Println("Usage: mmbaltracker <market>")
...
func getAvailableBalances(mkt *mm.MarketWithHost) (bals *balances, err error) {
	cmd := exec.Command("dexcctl", "mmavailablebalances", "/Users/marci/git/dcrdex/client/mm/sample-basic.json",
...
func main() {
	...
	for i := 1; i < len(os.Args); i++ {
		...
		currBalances[*mkt], err = getAvailableBalances(mkt)
		if err != nil {
	...
	checkForDiffs := func(mkt *mm.MarketWithHost) {
		newBals, err := getAvailableBalances(mkt)
		if err != nil {
			log.Errorf("Error getting balances: %v\n", err)
			return
		}

		logDiff := func(assetID uint32, oldBal, newBal uint64, dexOrCex string) {
			if oldBal != newBal {
				log.Infof("Market %s %s balance for asset %d changed from %d to %d",
					mkt, dexOrCex, assetID, oldBal, newBal)
			}
		}

		for assetID, newBal := range newBals.DEXBalances {
			oldBal := currBalances[*mkt].DEXBalances[assetID]
			logDiff(assetID, oldBal, newBal, "DEX")
		...
		for assetID, newBal := range newBals.CEXBalances {
			...
			logDiff(assetID, oldBal, newBal, "CEX")
