...
func (a *simpleArbMarketMaker) rebalance(newEpoch uint64) {
	...
	if exists {
		// Execution will not happen if it causes a self-match. Orders that
...
func (a *simpleArbMarketMaker) arbExistsOnSide(sellOnDEX bool, cexBaseBalance, cexQuoteBalance uint64) (exists bool, lotsToArb, dexRate, cexRate uint64) {
	...
	if lotsToArb > 0 {
		a.log.Infof("returning arb opportunity - sellOnDex: %v, lotsToArb: %v, dexRate: %v, cexRate: %v", sellOnDEX, lotsToArb, dexRate, cexRate)
...
func (a *simpleArbMarketMaker) executeArb(sellOnDex bool, lotsToArb, dexRate, cexRate, epoch uint64) {
	...
	dexOrder, err := a.core.Trade([]byte{}, &core.TradeForm{
		Host:    a.host,
		IsLimit: true,
		Sell:    sellOnDex,
		Base:    a.base,
