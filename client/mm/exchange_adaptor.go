...
func (u *unifiedExchangeAdaptor) MultiTrade(form *multiTradeForm) ([]*core.Order, error) {
	enough, err := u.sufficientBalanceForMultiTrade(form.host, form.baseID, form.quoteID, form.sell, form.placements, form.options)
	if err != nil {
		return nil, err
	}
	if !enough {
	...
	fromAsset := form.quoteID
	if form.sell {
		fromAsset = form.baseID
	}
