...
type SwapContract struct {
	Address common.Address // Only used for v0 contracts
	Gas     Gases
}
...
var Tokens = map[uint32]*Token{
	...
	polygonTokenID: {
		...
		NetTokens: map[dex.Network]*NetToken{
			...
			dex.Testnet: {
				Address: common.HexToAddress("0x44499312f493f62f2dfd3c6435ca3603ebfceeba"),
				SwapContracts: map[uint32]*SwapContract{
					1: {
						Gas: Gases{
							// Gas values are from usdt.eth mainnet.
...
func MaybeReadSimnetAddrsDir(
	...
	entryPoints map[dex.Network]common.Address,
	usdcToken *NetToken,
	usdtToken *NetToken,
) {
	...
	entryPointAddrFile := filepath.Join(harnessDir, "entrypoint_contract_address.txt")
	...
	entryPoints[dex.Simnet] = maybeGetContractAddrFromFile(entryPointAddrFile)
