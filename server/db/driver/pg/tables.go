...
func createMarketArchivedCommitIndexes(db sqlQueryExecutor, marketSchema string) error {
	for _, idx := range []indexStmt{
		{indexArchivedOrdersCommitName, internal.CreateArchivedCommitIndex},
		{indexArchivedCancelsCommitName, internal.CreateArchivedCommitIndex},
	} {
		table := marketSchema + "." + ordersArchivedTableName
		if idx.idxName == indexArchivedCancelsCommitName {
			table = marketSchema + "." + cancelsArchivedTableName
		}
		if err := createIndexStmt(db, idx.stmt, idx.idxName, table); err != nil {
			return err
		}
	}
	return nil
